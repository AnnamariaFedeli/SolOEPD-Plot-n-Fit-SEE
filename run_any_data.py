...
def run_all(path, data, savefig, plot_title = '', x_label = 'Intensity [/]', y_label = 'Energy [MeV]', legend_title = '', which_fit = 'best', e_min = None, e_max = None, g1_guess = -1.9, g2_guess = -2.5, g3_guess = -4, c1_guess = 1000, alpha_guess = 10, beta_guess = 10, break_guess_low = 0.6, break_guess_high = 1.2, cut_guess = 1.2, use_random = True, iterations = 20 , legend_details = False):
    """_summary_
    ...
    x_data = data['x'] # energy for spectra
    x_data_err  = data['x error']
    y_data   = data['y']
    y_data_err    = data['y error']
    ...
    ax.errorbar(x_data, y_data, yerr=y_data_err, xerr = x_data_err, marker='o', markersize= 3 , linestyle='', color='darkorange', alpha = 0.5, label='Data', zorder = -1)

    x_range_min = data['x'][0]
    x_range_max = data['x'][len(data['x'])-1]
