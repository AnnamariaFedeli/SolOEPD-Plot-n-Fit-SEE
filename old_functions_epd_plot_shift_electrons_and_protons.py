...
def extract_particle_data(df_electrons_or_protons, df_energies, plotstart, plotend, t_inj, species='electron',
    bgstart=None, bgend=None, bg_distance_from_window='2h', bg_period='60min', travel_distance=0,
    travel_distance_second_slope=None, fixed_window=None, instrument='ept', data_type='l2', averaging=None,
    masking=True, ion_conta_corr=False, df_protons=None, centre_pix=False):
    """
    Extract electron or proton fluxes and determine energy-dependent peak intensity and average 
    ...
    if instrument != 'step':
        df_particle_fluxes = (df_electrons_or_protons[f'{particle_name}_Flux'][plotstart:plotend])
        df_particle_uncertainties = (df_electrons_or_protons[f'{particle_name}_Uncertainty'][plotstart:plotend])

    # EPT proton data are needed for electron ion-contamination correction.
        ...
        if species == 'electron':
            ...
            if centre_pix:
                ...
                if 'Electron_Bins_Text' in df_energies.keys():
                    ...
                    raise ValueError('Could not identify the electron STEP energy information.')
            ...
            if data_type == 'l2':
                ...
                for i in channels:
                    ...
                    if centre_pix:
                        ...
                        df_particle_fluxes[f'Electron_Flux_{i}'] = (df_electrons_or_protons[f'Electron_Comb_Flux_{i}'][plotstart:plotend])
                        df_particle_uncertainties[f'Electron_Uncertainty_{i}'] = (df_electrons_or_protons[f'Electron_Comb_Uncertainty_{i}'][plotstart:plotend])

                    else:

                        df_particle_fluxes[f'Electron_Flux_{i}'] = (df_electrons_or_protons[f'Electron_Avg_Flux_{i}'][plotstart:plotend])
                        df_particle_uncertainties[f'Electron_Uncertainty_{i}'] = (df_electrons_or_protons[f'Electron_Avg_Uncertainty_{i}'][plotstart:plotend])
    ...
    if averaging is not None:
        ...
        if instrument != 'step':
            ...
            df_particle_fluxes = (df_particle_fluxes.resample(averaging).mean())

            df_particle_uncertainties = (df_particle_uncertainties.resample(averaging).apply(average_flux_error))
    ...
    if (species == 'electron' and instrument == 'ept' and ion_conta_corr):

        ion_cont_corr_matrix = np.loadtxt('EPT_ion_contamination_flux_paco.dat')

        electron_flux_cont = np.zeros(np.shape(df_particle_fluxes))
        electron_uncertainty_cont = np.zeros(np.shape(df_particle_uncertainties))
        ...
        df_particle_fluxes = (df_particle_fluxes - electron_flux_cont)
        df_particle_uncertainties = np.sqrt(df_particle_uncertainties**2 + electron_uncertainty_cont**2)
    ...
    primary_energies = [np.sqrt(e_low[i] * e_high[i]) for i in range(len(e_low))]

    df_info['Primary_energy'] = [primary_energies[i] for i in channels]

    energy_error_low = [primary_energies[i] - e_low[i] for i in range(len(primary_energies))]
    energy_error_high = [e_high[i] - primary_energies[i] for i in range(len(primary_energies))]

    df_info['Energy_error_low'] = [energy_error_low[i] for i in channels]
    df_info['Energy_error_high'] = [energy_error_high[i] for i in channels]

   
    # Particle velocity and search windows
    # ------------------------------------

    velocity = [evolt2speed(energy, particle_mass) for energy in primary_energies]

    travel_distance_km = (travel_distance * 1.496E8)

    DV = [travel_distance_km / v for v in velocity]

    searchstart = [pd.to_datetime(t_inj) + pd.Timedelta(seconds=dv) for dv in DV]
    searchend = []

    if fixed_window is None:

        travel_distance_second_slope_km = (travel_distance_second_slope * 1.496E8)

        DV2 = [travel_distance_second_slope_km / v for v in velocity]

        searchend = [pd.to_datetime(t_inj) + pd.Timedelta(seconds=dv) for dv in DV2]

    else:

        searchend = [start + pd.to_timedelta(fixed_window) for start in searchstart]
    ...
    if bg_distance_from_window is None:
        ...
        for start in searchstart:
            ...
            end = (start - pd.to_timedelta(bg_distance_from_window))

            bgend.append(end)
            bgstart.append(end - pd.to_timedelta(bg_period))
    ...
    for n, channel in enumerate(channels):
        ...
        flux_column = (f'{particle_name}_Flux_{channel}')
        uncertainty_column = (f'{particle_name}_Uncertainty_{channel}')

        particle_flux = (df_particle_fluxes[flux_column])
        particle_uncertainty = (df_particle_uncertainties[uncertainty_column])

        # Peak/search window
        # ------------------

        f_p = particle_flux[searchstart[n]:searchend[n]]
        ...
        if len(f_p) == 0:
            ...
            bg_flux = particle_flux[bgstart[n]:bgend[n]].mean(skipna=True)
        ...
        if (pd.isna(peak_timestamp) or len(particle_uncertainty) == 0):
            peak_uncertainty = np.nan

        else:
            timestamp_loc = (particle_uncertainty.index.get_indexer([peak_timestamp], method='nearest')[0])
            peak_uncertainty = (particle_uncertainty.iloc[timestamp_loc])

        list_peak_uncertainties.append(peak_uncertainty)

        # Average background uncertainty
        # ------------------------------

        bg_uncertainty = particle_uncertainty[bgstart[n]:bgend[n]]
        ...
        bg_std = particle_flux[bgstart[n]:bgend[n]].std()
        list_bg_std.append(bg_std)

        # Average flux in search window
        # -----------------------------

        f_a = particle_flux[searchstart[n]:searchend[n]]
    ...
    for i in range(len(list_flux_peaks)):
        ...
        bg_subtracted_peak = (list_flux_peaks[i] - list_bg_fluxes[i])
        list_bg_subtracted_peaks.append(bg_subtracted_peak)
        list_peak_significance.append(bg_subtracted_peak / list_bg_std[i])

        if bg_subtracted_peak < list_bg_fluxes[i]:
            list_peak_significance[i] = -1

        bg_subtracted_average = (list_flux_average[i] - list_bg_fluxes[i])
        list_bg_subtracted_average.append(bg_subtracted_average)
        list_average_significance.append(bg_subtracted_average / list_bg_std[i])

        if (bg_subtracted_average < list_bg_fluxes[i]):
    ...
    df_info[f'Peak_{species}_uncertainty'] = list_peak_uncertainties
    df_info[f'Bg_{species}_uncertainty'] = list_average_bg_uncertainties
    ...
    df_info['Backsub_peak_uncertainty'] = np.sqrt(df_info[f'Peak_{species}_uncertainty'] ** 2 + df_info[f'Bg_{species}_uncertainty'] ** 2)
    ...
    return (df_particle_fluxes, df_info, [searchstart, searchend], [e_low, e_high], [instrument, data_type])
...
def extract_proton_data(df_protons, df_energies, plotstart, plotend, t_inj, bgstart=None, bgend=None, bg_distance_from_window='120min', bg_period='60min',
    ...
    if instrument == 'ept':
        ...
        if data_type == 'll':
            ...
            for i in channels:
                ...
                df_proton_fluxes = df_proton_fluxes.rename(columns={f'H_Flux_{i}': f'Ion_Flux_{i}'})
                df_proton_uncertainties = df_proton_uncertainties.rename(columns={f'H_Flux_Sigma_{i}': f'Ion_Uncertainty_{i}'})
        ...
        if data_type == 'll':
            ...
            for i in channels:
                ...
                df_proton_fluxes = df_proton_fluxes.rename(columns={f'H_Flux_{i}': f'Ion_Flux_{i}'})
                df_proton_uncertainties = df_proton_uncertainties.rename(columns={f'H_Uncertainty_{i}': f'Ion_Uncertainty_{i}'})
        ...
        if data_type == 'l2':
            ...
            for i in channels:
                ...
                df_proton_fluxes[f'Ion_Flux_{i}'] = (df_protons[f'Magnet_Avg_Flux_{i}'][plotstart:plotend])
                df_proton_uncertainties[f'Ion_Uncertainty_{i}'] = (df_protons[f'Magnet_Avg_Uncertainty_{i}'][plotstart:plotend])
    ...
    for n, channel in enumerate(channels):
        proton_flux = df_proton_fluxes[f'Ion_Flux_{channel}']
        proton_uncertainty = df_proton_uncertainties[f'Ion_Uncertainty_{channel}']
        ...
        if len(b_f) == 0:
            ...
            bg_flux = proton_flux[bgstart[n]:bgend[n]].mean(skipna=True)
        ...
        if pd.isna(peak_timestamp):
            ...
            timestamp_loc = proton_uncertainty.index.get_indexer([peak_timestamp],method='nearest')[0]
            peak_proton_uncertainty = (proton_uncertainty.iloc[timestamp_loc])
            list_peak_proton_uncertainties.append(peak_proton_uncertainty)

        # Average uncertainty in the background window.
        bg_uncertainty = proton_uncertainty[bgstart[n]:bgend[n]]
        average_bg_uncertainty = (np.sqrt((bg_uncertainty ** 2).sum()) / len(bg_uncertainty))
        list_average_bg_uncertainties.append(average_bg_uncertainty)

        # Standard deviation of the background flux.
        bg_std = proton_flux[bgstart[n]:bgend[n]].std()
        list_bg_std.append(bg_std)
...
def plot_channels(args,species='electron', bg_subtraction=False, savefig=False, sigma=3,path='', key='', frac_nan_threshold=0.4, 
                  rel_err_threshold=0.5, plot_pa=False, coverage=None, viewing='sun', centre_pix=False, date=None,size=2 ):
    """
    ...
    title_string = (instrument.upper()+ ', '+ species.upper()+ 'S, ' + data_type.upper() + ', ' + date_string)

    filename = ( species + '_channels-'+ file_date + '-' + instrument.upper() + '-' + viewing + '-' + data_type.upper())
    ...
    if df_info['Averaging'][0] == 'Mean':
        ...
        title_string = (title_string + ', ' + averaging + ' averaging')
        filename = (filename + '-' + averaging + '_averaging')
    ...
    if bg_subtraction:
        ...
        df_fluxes = df_fluxes.sub(df_info['Background_flux'].values,axis=1)
    ...
    for n, channel in enumerate(df_info['Energy_channel']):
        ...
        ax = axes[n]
        ax.plot(df_fluxes.index,df_fluxes[f'{flux_column_prefix}_{channel}'], color=color[viewing], drawstyle='steps-mid')
        ax.set_yscale('log')
        ...
        ax.set_xlim(df_fluxes.index[0],df_fluxes.index[-1])
        ...
        if (n == len(df_info['Energy_channel']) - 1 and not plot_pa):
...
def plot_channels_electrons(args, bg_subtraction=False, savefig=False, sigma=3, path='', key='', frac_nan_threshold=0.4, 
...
def plot_channels_protons(args, bg_subtraction=False, savefig=False, sigma=3, path='', key='', frac_nan_threshold=0.4,
                          rel_err_threshold=0.5, plot_pa=False, coverage=None, viewing='sun', date=None, size=20):
    """
    ...
    title_string = (instrument.upper() + ', PROTONS, ' + data_type.upper() + ', ' + date_string)

    filename = ('proton_channels-' + file_date+ '-' + instrument.upper() + '-' + viewing + '-' + data_type.upper())
...
viewing='sun', frac_nan_threshold=0.9, rel_err_threshold=0.5, channels=None, figsize_x=15, figsize_y=8, f_scale=1, f_size=12,
species='electron', centre_pix=False ):
    """
    ...
    title_string = (instrument.upper() + ', ' + species.upper()+ ', ' + data_type.upper() + ', ' + str(df_info['Plot_period'][0][:-5]))
    filename = (species + '_channels-' + str(df_info['Plot_period'][0][:-5]) + '-' + instrument.upper()+ '-' + data_type.upper())
    ...
    for n, channel in enumerate(channels, start=1):
        ...
        ax = df_fluxes['{}_{}'.format(flux_prefix, channel)].plot(logy=True, figsize=(figsize_x, figsize_y), color='red', drawstyle='steps-mid')
        ...
        if df_info['Peak_timestamp'][channel_index] is not pd.NaT:
            ...
            if (peak_sig[channel_index] >= sigma and rel_err[channel_index] <= rel_err_threshold and df_info['frac_nonan'][channel_index] > frac_nan_threshold):
...
def plot_some_channels_electrons(args, bg_subtraction=False, savefig=False, sigma=3, path='', key='', plot_pa=False, coverage=None, 
...
def plot_spectrum_peak(args,species, bg_subtraction=True, savefig=False, path='', key='', sigma=3,
    frac_nan_threshold=0.4, rel_err_threshold=0.5, direction=None, centre_pix=False, date=None):
    """
    ...
    filename = (f"{species}_spectrum-{file_date}-{instrument.upper()}{viewing}-{data_type.upper()}")
    ...
    if bg_subtraction:
        ax.errorbar(x=df_info['Primary_energy'], y=df_info['Bg_subtracted_peak'], yerr=df_info['Backsub_peak_uncertainty'], xerr=[df_info['Energy_error_low'], df_info['Energy_error_high']], color=color[direction], fmt='o', ecolor=color[direction], zorder=0, label='Flux peaks')
        ...
        ax.errorbar(x=df_info['Primary_energy'], y=df_info['Flux_peak'], yerr=df_info[f'Peak_{species}_uncertainty'], xerr=[df_info['Energy_error_low'], df_info['Energy_error_high']], fmt='o', color=color[direction], ecolor=color[direction], zorder=0, label='Intensity peaks')
    ...
    ax.errorbar(x=df_info['Primary_energy'], y=df_info['Background_flux'], yerr=df_info[f'Bg_{species}_uncertainty'], xerr=[df_info['Energy_error_low'], df_info['Energy_error_high']], fmt='o', color=color[direction], ecolor=color[direction], alpha=0.15, label='Background intensity')
...
def plot_spectrum_peak_electrons(args, bg_subtraction=True,
...
def plot_spectrum_average(args,species, bg_subtraction=True, savefig=False, path='', key='', sigma=3,
frac_nan_threshold=0.4, rel_err_threshold=0.5, direction=None, centre_pix=False, date=None):
    """
    ...
    if bg_subtraction:
        ax.errorbar(x=df_info['Primary_energy'], y=df_info['Bg_subtracted_average'], yerr=df_info['Backsub_peak_uncertainty'], xerr=[df_info['Energy_error_low'],df_info['Energy_error_high']], color=color[direction], fmt='o', ecolor=color[direction], zorder=0, label='Intensity average')
        ...
        ax.errorbar(x=df_info['Primary_energy'], y=df_info['Flux_average'], yerr=df_info[f'Peak_{species}_uncertainty'], xerr=[df_info['Energy_error_low'], df_info['Energy_error_high']], fmt='o', color=color[direction], ecolor=color[direction], zorder=0, label='Intensity average')
...
def plot_spectrum_average_electrons(args, bg_subtraction=True, savefig=False, path='', key='', sigma=3, frac_nan_threshold=0.4, 
...
def write_to_csv(args, date, species, path='', key='', direction=None, centre_pix=False):
    """
...
def write_to_csv_electrons(args, date, path='', key='', direction=None, centre_pix=False):
    """
    Saves the spectrum information dataframe to a CSV file.
